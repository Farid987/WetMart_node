...
module.exports.controller = (app, io, socket_list) => {
    ...
    app.post("/api/admin/product_add", (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            ...
            form.parse(req, (err, reqObj, files) => {
                ...
                helper.CheckParameterValid(res, reqObj, ["name", "detail", "cat_id", "type_id", "unit_name", "unit_value", "nutrition_weight", "price", "nutrition_data"], () => {
                    helper.CheckParameterValid(res, files, ["image"], () => {
                        ...
                        db.query("INSERT INTO `product_detail`(`cat_id`, `type_id`, `name`, `detail`, `unit_name`, `unit_value`, `nutrition_weight`, `price`, `created_date`, `modify_date`) VALUES (?,?,?, ?,?,?, ?,?,?, NOW(), NOW() ) ", [reqObj.cat_id[0], reqObj.type_id[0], reqObj.name[0], reqObj.detail[0], reqObj.unit_name[0], reqObj.unit_value[0], reqObj.nutrition_weight[0], reqObj.price[0]], (err, result) => {
                            if (err) {
                                helper.ThrowHtmlError(err, res);
                                return
                            }
    ...
    app.post('/api/admin/product_update', (req, res) => {
        ...
        helper.CheckParameterValid(res, reqObj, ["prod_id", "name", "detail", "cat_id", "type_id", "unit_name", "unit_value", "nutrition_weight", "price"], () => {

            checkAccessToken(req.headers, res, (uObj) => {

                db.query("UPDATE `product_detail` SET `cat_id`=?,`type_id`=?,`name`=?,`detail`=?,`unit_name`=?,`unit_value`=?,`nutrition_weight`=?,`price`=?, `modify_date`=NOW() WHERE  `prod_id`= ? AND `status` = ? ", [
                    reqObj.cat_id, reqObj.type_id, reqObj.name, reqObj.detail, reqObj.unit_name, reqObj.unit_value, reqObj.nutrition_weight, reqObj.price, reqObj.prod_id, "1"
                ], (err, result) => {
    ...
    function getProductDetail(res, prod_id) {
        db.query("SELECT `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`type_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, `pd`.`created_date`, `pd`.`modify_date`, `cd`.`cat_name`, `td`.`type_name` FROM `product_detail` AS  `pd` " +
            "INNER JOIN `category_detail` AS `cd` ON `pd`.`cat_id` = `cd`.`cat_id` " +
            "INNER JOIN `type_detail` AS `td` ON `pd`.`type_id` = `td`.`type_id` " +
    ...
    app.post('/api/admin/offer_product_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            ...
            db.query('SELECT  `od`.`offer_id`, `od`.`prod_id`, `od`.`price` AS `offer_price`, `od`.`start_date`, `od`.`end_date`, `od`.`status`, `od`.`created_date`, `od`.`modify_date`, `pd`.`cat_id`, `pd`.`type_id`, `pd`.`name`, `pd`.`detail`, `pd`.`price`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, IFNULL( `cd`.`cat_name`, "" ) AS `cat_name`, IFNULL( `td`.`type_name`, "" ) AS `type_name`, (CASE WHEN `imd`.`image` != "" THEN CONCAT( "' + image_base_url + '", "", `imd`.`image` ) ELSE "" END) AS `image` FROM `offer_detail` AS `od` ' +
